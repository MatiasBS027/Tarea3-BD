...
type PlanillaSemanalRow = {
    ...
    SalarioNeto: number;
    QHorasOrdinarias: number;
    QHorasExtraNormales: number;
    QHorasExtraDobles: number;
};

type DeduccionSemanalRow = {
    idPlanillaSemanal: number;
    idTipoDeduccion: number;
...
class EmpleadoViewPage {
    ...
    private readonly semanalContenido: HTMLElement;
    private readonly mensualEstado: HTMLElement;
    private readonly mensualContenido: HTMLElement;
    ...
    constructor() {
        ...
        this.semanalContenido = document.getElementById('semanalContenido') as HTMLElement;
        this.mensualEstado    = document.getElementById('mensualEstado') as HTMLElement;
        this.mensualContenido = document.getElementById('mensualContenido') as HTMLElement;
    ...
    // ----------------------------------------------------------------
    private async cargarPlanillaSemanal(): Promise<void> {
        ...
        try {
            ...
            const response = await fetch(
                `/api/planilla/semanal/${this.empleadoId}?cantidadSemanas=10`,
                { headers }
            );
            const payload = await response.json() as {
                success: boolean;
                message?: string;
                data?: {
                    planillas: PlanillaSemanalRow[];
                    deducciones: DeduccionSemanalRow[];
                    asistencias: AsistenciaDiariaRow[];
                };
            };
    ...
    private renderPlanillaSemanal(
        ...
        if (deducciones.length > 0) {
            const porSemana = new Map<number, DeduccionSemanalRow[]>();
            for (const d of deducciones) {
                if (!porSemana.has(d.idPlanillaSemanal)) porSemana.set(d.idPlanillaSemanal, []);
        ...
        if (asistencias.length > 0) {
            const porSemana = new Map<number, AsistenciaDiariaRow[]>();
            for (const a of asistencias) {
                if (!porSemana.has(a.idPlanillaSemanal)) porSemana.set(a.idPlanillaSemanal, []);
    ...
    // ----------------------------------------------------------------
    private async cargarPlanillaMensual(): Promise<void> {
        ...
        try {
            ...
            const response = await fetch(
                `/api/planilla/mensual/${this.empleadoId}?cantidadMeses=6`,
                { headers }
            );
            const payload = await response.json() as {
                success: boolean;
                message?: string;
                data?: {
                    planillas: PlanillaMensualRow[];
                    deducciones: DeduccionMensualRow[];
                    semanas: SemanaMensualRow[];
                };
            };
    ...
    private renderPlanillaMensual(
        ...
        if (deducciones.length > 0) {
            const porMes = new Map<number, DeduccionMensualRow[]>();
            for (const d of deducciones) {
                if (!porMes.has(d.idPlanillaMensual)) porMes.set(d.idPlanillaMensual, []);
        ...
        if (semanas.length > 0) {
            const porMes = new Map<number, SemanaMensualRow[]>();
            for (const s of semanas) {
                if (!porMes.has(s.idPlanillaMensual)) porMes.set(s.idPlanillaMensual, []);
